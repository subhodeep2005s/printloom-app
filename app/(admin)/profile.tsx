import { useForgotPassword, useLogout, useMe, useResetPassword } from "@/app/shared/api/auth.query";
...
function ChangePasswordModal({ email, onClose }: { email: string; onClose: () => void }) {
  ...
  const handleSendOtp = () => {
    ...
    sendResetCode(
      ...
      {
        onSuccess: () => {
          toast.show({ type: "success", title: "Reset code sent", message: "Check your email for the reset code." });
          setStep("verify");
        },
        onError: (err: any) => {
          toast.show({
